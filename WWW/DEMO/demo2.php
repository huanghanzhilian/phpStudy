...
@
  $goods_content=iconv('gb2312','utf-8',odbc_result($quee,22));//商品描述
@
 $goods_color=iconv('gb2312','utf-8',odbc_result($quee,29));//颜色
 $shop_card=iconv('gb2312','utf-8',odbc_result($quee,41));//店铺ID
 $goods_card=iconv('gb2312','utf-8',odbc_result($quee,42));//货品ID
 //$goods_img=iconv('gb2312','utf-8',odbc_result($quee,47)).'47';//图片
$sku_name=iconv('gb2312','utf-8',odbc_result($quee,49));//sku-id
$order_number=iconv('gb2312','utf-8',odbc_result($quee,50));//子订单号
...
if($arr==''){
//echo 1;exit;
$sql="insert into dingdan(username,usernamee,phone,post_code,country,province,city,district,address,logistics_number,goods_name,goods_na,goods_number
,goods_price,goods_nums,discount_price,sum_price,goods_content,goods_color,shop_card,goods_card,qibiao,forum,order_state
,revise_time,order_number,order_time,zt,sku_name)values('$username','$usernamee','$phone','$post_code','$country','$province','$city','$district','$address','$logistics_number','$goods_name','$goods_na','$goods_number','$goods_price','$goods_nums','$discount_price','$sum_price','$goods_content','$goods_color','$shop_card','$goods_card','$qibiao','$forum','$order_state','$revise_time','$order_number','$order_time','1','$sku_name')";
		
		$re=mysql_query($sql);
		if($re){
		$mysqll="select * from dingdan where p_id='$re'";
