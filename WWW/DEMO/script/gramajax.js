	...
	function resdpmsd(dser,pia){
		...
		if(pia=="a"){
			paginat.innerHTML="<div>"+dser.p_id+"</div>"+"<div>"+dser.goods_name+"</div>"+"<div>"+dser.goods_content+"</div>"+"<div class='buttons'><span onclick='xiadan(this,"+dser.p_id+")'>下单</span>"+"<span onclick='fsdx(this,dser.p_id)'>发送短信</span>"+"<span onclick='biji(this)'>笔记</span></div>"
		}else if(pia=="b"){
	...
	function redcdpm(dser){
		//console.log(dser)
		// if(dser.p_id=="undefined"){
		// 	console.log(dser.p_id)
		// }
		...
		request.onreadystatechange = function() {
			if (request.readyState === 4) {
				if (request.status === 200) {
					...
					if(request.responseText){
						console.log(request.responseText)
						var el=_this.parentNode.parentNode;
	...
	//下单
	function xiadan(_this,p_id){
		var el=_this.parentNode.parentNode;
		var request = new XMLHttpRequest();
		request.open("POST", "think/index.php/home/index/xiadan");
		var data = "p_id="+p_id;
		request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		request.send(data);
		request.onreadystatechange = function() {
