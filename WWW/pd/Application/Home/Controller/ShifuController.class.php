...
class ShifuController extends Controller {
	...
	public function shifu_map(){
		...
		$username=$_COOKIE['username'];
			...
			$mysql=M('goods');
			$arr=$mysql->where("stau='0' and ways_id='1'")->order("id desc")->select();
			//print_r($arr);die;
		}
		}
			$this->assign('arr',$arr);
	...
	$arr=$mysql
		...
		->where("(stau='1' and user_id='$user_id') or (ways_id='1' and stau='0')")->order('id desc')->select();
		}
	$arra=$mysql
...
//echo $xiugai;die;
	...
	public function chongzhi(){
			...
			if($cangku_id){
				 $pp['zta']=0;
				$sqll->where("cangku_id='$cangku_id'")->save($pp);
			}
			$sql=M('zt_time');
