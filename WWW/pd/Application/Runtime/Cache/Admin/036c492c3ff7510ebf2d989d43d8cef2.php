...
<div class="header public_width">
    <h1 class="header_h1">产品库 &gt; 手机维修 &gt; 机型回收站</h1>
</div>
        ...
        <ul class="zhuti_rom">
            <?php if(is_array($arr)): foreach($arr as $key=>$kk): ?><li class="">
                    <div class="zhuti_rom_list_r fr zhuti_rom_list_ncon">
                        <a class="Return_button_a1" href="/Admin/Shangpin/brandhuifu/brand_id/<?php echo ($kk["brand_id"]); ?>/type_id/<?php echo ($type_id); ?>" onclick="if(confirm('确定恢复?')==false)return false;">恢复</a>
                    </div>
                    <strong class="buctrese"><?php echo ($kk["brand_name"]); ?></strong>
