...
<style type="text/css">
.vbcxv { width:55px;}
.vbcxv_pom{ width:125px; display:inline-block; overflow:hidden; display:block; margin-right:15px;
overflow: hidden;
    white-space: nowrap;
...
<div class="header public_width">
    <h1 class="header_h1">产品库 &gt; 手机维修 &gt; 苹果 &gt; 问题 &gt; 小问题修改
    <a class="youb_mk" href="/Admin/Shangpin/questionhs/model_id/<?php echo ($model_id); ?>/brand_id/<?php echo ($brand_id); ?>/questiontype_id/<?php echo ($questiontype_id); ?>">回收</a>
    </h1>
</div>
...
<div class="container public_width">
    ...
    <div class="Return">
        ...
        <a class="Return_button" href="/Admin/Shangpin/xiaowenti/brand_id/<?php echo ($brand_id); ?>/model_id/<?php echo ($model_id); ?>/question_id/<?php echo ($question_id); ?>">返回</a>
    </div>
    <!--公共返回-->
    <div class="append_cp">
        <div class="append_cp_box clearfix">
            <button class="append_cp_anniu_but" id="tianco">添加问题</button>
    ...
    <div class="container_modify" id="xgpinpan">
        <ul class="zhuti_rom" id="p">
            <?php if(is_array($arr)): foreach($arr as $key=>$k): ?><li class="" id="<?php echo ($k["question_id"]); ?>" style="padding:0;">
    ...
    <div class="bikuss" id="bikuss">
        <ul class="zhuti_rom zhuti_rom_a1">
            <form method="post" action="/Admin/Shangpin/questtadd" onSubmit="return Checked();">
                
                <h2 class="erji_h2">请选择您要添加的问题</h2>
                <li class="">
                    <?php if(is_array($arr5)): foreach($arr5 as $key=>$ll): ?><input class="color_ya_input" type="checkbox" name="question_id[]" value='<?php echo ($ll["xwt_id"]); ?>'>
                        <span class="Return_button_a1 Return_button_a2 color_ya"><?php echo ($ll["xwt_name"]); ?></span>
                        <input class="color_ya_input" type="checkbox" name="question_name[]" value='<?php echo ($ll["xwt_name"]); ?>'><?php endforeach; endif; ?>
                </li>
	        	...
	        	<input type="hidden" id='questiontype_id' name="questiontype_id" value='<?php echo ($questiontype_id); ?>'>
...
eibur()
	function as1(id,thise){
       ...
       var questionid=shang_id.attr('id');
	   
	   var brandid=document.getElementById('brand_id').value;
	   var questiontype_id=document.getElementById('questiontype_id').value;
	   var model_id=document.getElementById('model_id').value;
	   
		var ajax=new XMLHttpRequest()
			ajax.open("get","/Admin/Shangpin/questionpai/model_id/"+model_id+"/questionid/"+questionid+"/brand_id/"+brandid+"/questiontype_id/"+questiontype_id+"/question_id/"+id,true)
			ajax.send()
...
function ass(){
	...
	var questiontype_id=document.getElementById('questiontype_id').value;
		var ajax=new XMLHttpRequest()
			ajax.open("get","/Admin/Shangpin/jiejuefangan/question_id/"+id+"/model_id/"+model_id+"/brand_id/"+brand_id+"/questiontype_id/"+questiontype_id,true)
			ajax.send()
			ajax.onreadystatechange=function(){
				if(ajax.readyState==4 && ajax.status==200){
					document.getElementById('p').innerHTML=ajax.responseText
