...
function removepage(){
	...
	while (pagecount.hasChildNodes()) {pagecount.removeChild(pagecount.lastChild);}//清空
}
	...
	request.onreadystatechange = function() {
		if (request.readyState === 4) {
			if (request.status === 200) {
				...
				var pama = ["city","id2","id3","id4"];
                pagetool = new Pageglobal(data,pama,"main-center","pagecount");
                pagetool.init();
...
function beizhuse(pia,value) {
		//清空页
  	removepage()
	var request = new XMLHttpRequest();
	request.open("POST", "http://m.weibaishi.com/admin/aingdan/bijia");
	var data = "id="+pia+"&question="+value;
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send(data);
	request.onreadystatechange = function() {
		if (request.readyState === 4) {
...
function yiquxiao() {
	var main_center = document.getElementById("main-center")
	main_center.innerHTML = '<ul id="nav_top" class="tabsd flex-wrap">\
			<li data_type="yiquxiao1" class="flex-con active">确认取消原因</li>\
			<li data_type="yiquxiao2" class="flex-con">取消单处理</li>\
		</ul>';
	yiquxiao1();
	var nav_top= document.getElementById("nav_top");
	var tab_li = nav_top.getElementsByTagName("li");
...
function yiquxiao2() {
	...
	var dege = ["序号", "取消时间", "接单工程师", "服务项目", "服务地址", "客户姓名","客户电话"]
	var request = new XMLHttpRequest();
	request.open("POST", "http://m.weibaishi.com/admin/aingdan/quxiaole");
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send();
	request.onreadystatechange = function() {
		if (request.readyState === 4) {
			if (request.status === 200) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					chsera.innerHTML = '<td>' + (i + 1) + '</td>\
		        	<td>' + data[i].time+'</td>\
					<td>' + data[i].sf_username + '</td>\
					<td>' + data[i].u_title + '</td>\
					<td>' + data[i].address + '</td>\
					<td>\
					<input data="'+data[i].id+'" class="sdcmkj"  type="text" name="brand_name" value="' + data[i].user_name + '">\
					</td>\
					<td>' + data[i].phone + '</td>';
					chserac.innerHTML='<tr>\
					<td colspan="8">\
					<div class="anniu-center">\
						<div class="anniu fl">\
								<p class="alerds">' + data[i].quxiao_name + '</p>\
								<span onclick="huifu('+data[i].id+')">订单恢复</span>\
								<span onclick="chongxuan('+data[i].id+',this)">订单恢复并且指派</span>\
								<span class="bijichu srebj" >笔记\
									<div style="display:none" class="tanbuks">\
										<div class="shuop">\
											<textarea data="'+data[i].id+'" class="shubhg_pcc" name="content" autocomplete="off" >'+data[i].question+'</textarea>\
										</div>\
	                                </div>\
								</span>\
						</div>\
				...
				$(".sdcmkj").change(function(){
					//alert($(this).attr("data"))
					var dcop=$(this).attr("data");
					$.ajax({
						url: "http://m.weibaishi.com/admin/aingdan/xg",
						data: {
							'id':dcop,
							'user_name': $(this).val(),
						},
...
function tijiao(c){
	$.ajax({
		url:"/Admin/Quyuguanli/xiugaichengshi",
		data: {'smwx_id':c.getAttribute('data'),'smwx_name':c.value},
		type: "post",
		//dataType: "html",
		success: function (data) {
			$(c).val(data);
...
function huifu(id){
	var request = new XMLHttpRequest();
	request.open("POST", "http://m.weibaishi.com/admin/aingdan/dingdanhuifu");
	var data = "id="+id;
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send(data);
	request.onreadystatechange = function() {
...
function yiquxiao1() {
	...
	var dege = ["序号", "取消时间", "接单工程师", "服务项目", "服务地址", "客户姓名","客户电话"]
	var request = new XMLHttpRequest();
	request.open("POST", "http://m.weibaishi.com/admin/aingdan/quxiao");
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send();
	request.onreadystatechange = function() {
		if (request.readyState === 4) {
			if (request.status === 200) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					<div class="anniu-center">\
						<div class="anniu fl">\
								<p class="alerds">' + data[i].quxiao_name + '</p>\
								<span onclick="querenquxiao('+data[i].id+')">确认原因</span>\
								<span onclick="chongxuan('+data[i].id+',this)">重选原因</span>\
						</div>\
					</div>\
					</td>\
					</tr>'
...
//确认取消
function querenquxiao(id){
	var request = new XMLHttpRequest();
	request.open("POST", "http://m.weibaishi.com/admin/aingdan/querenquxiao");
	var data = "id="+id;
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send(data);
	request.onreadystatechange = function() {
...
//重选列表
function chongxuan(id,_this){
	var request = new XMLHttpRequest();
	request.open("POST", "http://m.weibaishi.com/admin/aingdan/wt");
	//var data = "id="+id;
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send();
	request.onreadystatechange = function() {
...
//对话框形状自动调整重置取消列表
function xunhresize(data,_this,idd) {
	var overlay= document.getElementById("attr-overlay")
	var content= document.getElementById("attr-content");
	var widget_title=document.getElementById("widget-title");
	widget_title.innerHTML="匹配sku"
	...
	for (var i = 0; i < data.length; i++) {
		var btnspan = document.createElement("li");
		btnspan.className="";
		btnspan.innerHTML='<div class="left">'+data[i].quxiaole_name+'</div>\
		<div class="reft">\
		<span>\
		<input  type="radio" name="cangku_id" class="sd'+data[i].quxiaole_id+'" value="'+data[i].quxiaole_id+'">\
		</span>\
		</div>';
		btnspan.setAttribute("data",data[i].quxiao_id)
		ul.appendChild(btnspan);
	};
	content_sd.appendChild(ul);
	var button = document.createElement("button");
	...
	for (var i = 0; i < li.length; i++) {
		li[i].addEventListener('click', function(e) {
			e = e || window.event;
			yansesd(this,button,pia,_this)

		}, false);
	};*/
	$(".attr-content-con li").on("click",function(){
		$(this).find('input').attr("checked", true);

		if ($('input.sd10').attr('checked')) {
			alert(1)
			$(".comtext").show();
		} else {
			$(".comtext").hide();
		}
		$(".attr-content-button").css("background", "#5a8bed");
	})
	$(".attr-content-button").on("click",function(){
		var val = $('input:radio[name="cangku_id"]:checked').val();
		if (val) {
			var request = new XMLHttpRequest();
			request.open("POST", "http://m.weibaishi.com/admin/aingdan/chongxuan");
			var data = "id="+idd+"&quxiaole_id="+val+"&quxiaole_content=";
			request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			request.send(data);
			request.onreadystatechange = function() {
				if (request.readyState === 4) {
...
$(function() {
	...
	//事件委托
	/*bodyNode.delegate(".attrdfn", "click", function(e) {
		alert(1)
		$(this).find('input').attr("checked", true);
		...
		$("#subh").css("background", "#5a8bed");


		if($('input.chmn_dmk').attr('data')=="10"){
			alert(1)
		}else{
			alert(2)
		}
	})*/
	/*$(".attr-content-con").on("click", function() {
		alert(1)
	});*/
})
