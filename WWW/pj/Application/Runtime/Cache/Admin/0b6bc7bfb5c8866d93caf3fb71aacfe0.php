...
<div class="header public_width">
    <h1 class="header_h1">产品库 &gt; 手机维修 &gt; <?php echo ($brand_name); echo ($model_name); echo ($brand_id); ?>解决方案回收站</h1>
</div>
...
<div class="container public_width">
    ...
    <div class="Return">
        ...
        <a class="Return_button" href="/Admin/Shangpin/model/brand_id/<?php echo ($brand_id); ?>/model_id/<?php echo ($model_id); ?>">返回</a>
    </div>
    <!--公共返回-->
        ...
        <ul class="zhuti_rom">
            <?php if(is_array($arr)): foreach($arr as $key=>$kk): ?><li class="">
                    <div class="zhuti_rom_list_r fr zhuti_rom_list_ncon">
                        <a class="Return_button_a1" href="/Admin/Shangpin/planhuifu/brand_id/<?php echo ($brand_id); ?>/model_id/<?php echo ($model_id); ?>/plan_id/<?php echo ($kk["plan_id"]); ?>" onclick="if(confirm('确定恢复?')==false)return false;">恢复</a>
                    </div>
