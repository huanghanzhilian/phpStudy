...
<div class="header public_width">
    <h1 class="header_h1">产品库 &gt; 手机维修 &gt; 解决方案库</h1>
</div>
<div class="container public_width">
    ...
    <div class="append_cp">
        <div class="append_cp_box clearfix">
            <form method="post" action="/Admin/Shangpin/jiejueadd">
                <input class="append_cp_text fl" type="text" name="jiejue_name" placeholder="请填写您要添加的小问题"/>
    ...
    <div class="container_modify" id="xgpinpan">
        <ul class="zhuti_rom zhuti_rom_ll">
            <?php if(is_array($arr)): foreach($arr as $key=>$kk): ?><li class="">
                    <form method="post" action="/Admin/Shangpin/planup">
                        <div class=" fr ">
                        ...
                        <a class="zjnmk_pp" style="float:none; display:inline-block;" href="/Admin/Shangpin/jiejueshanchua/brand_id/<?php echo ($brand_id); ?>/jiejue_id/<?php echo ($kk["jiejue_id"]); ?>" onclick="if(confirm('确定删除?')==false)return false;"><span class="zhkkbn"></span></a>
                        <strong class="buctrese" style="width:500px; display:inline-block;">
                            <input style="width:100%; text-align:left;" readonly="true" class="xiubuttce xiubuttce_as1" type="text" name="jiejue_name" value='<?php echo ($kk["jiejue_name"]); ?>'>
                        </strong>
                        <input type="hidden" name="jiejue_id" value='<?php echo ($kk["jiejue_id"]); ?>'>
